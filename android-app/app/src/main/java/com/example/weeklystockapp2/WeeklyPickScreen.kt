...
fun WeeklyPickScreen(
    ...
    if (showInfoDialog) {
        AlertDialog(
            ...
            title = {
                Text(
                    text = "Hur score och risk beräknas",
                    style = MaterialTheme.typography.titleMedium,
                    fontWeight = FontWeight.SemiBold
                )
            },
            text = {
                Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    Text(
                        text = "Modellen tittar just nu på två saker för varje aktie:"
                    )
                    Text(
                        text = "• Momentum: prisförändring senaste 5 handelsdagarna."
                    )
                    Text(
                        text = "• Volatilitet: hur mycket kursen svänger dag till dag."
                    )
                    Text(
                        text = "Score = momentum − volatilitet. Högre score är bättre, " +
                                "för att vi gillar aktier som går upp men inte svänger för mycket."
                    )
                    Text(
                        text = "Risknivå:\n" +
                                "• Low: låg volatilitet\n" +
                                "• Medium: medelhög volatilitet\n" +
                                "• High: hög volatilitet\n\n" +
                                "Modellen är förenklad och ska inte ses som investeringsrådgivning."
                    )
                }
            },
            confirmButton = {
